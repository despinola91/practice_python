...
def run():
    ...
    print(" --- Guesser game --- ")
    start = input("\n\nAlright think of a number between 0 and 100 and I will try to guess it.\nReady? (y/n)\n")
    if start == "exit":
        exit()
    elif start.lower() == "y":
        answer = ""
        ...
        while answer != "c":
            guess = get_best_guess(low, high)
            answer = input("Please select an option for the following number: " + str(guess)+"\n\n\ta - The number is too high\n\tb - The number is too low\n\tc - The number is my number!\n")
            if answer == "exit":
                exit()
            if answer == "a":
                high = guess
            if answer == "b":
                low = guess
            if answer == "c":
                print("\nWoohoo!! It only took me " + str(guesses) + " tries!")
